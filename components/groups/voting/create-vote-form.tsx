        ...
        <Card className="w-full mb-6 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
            <CardHeader>
                <div className="flex items-center justify-between">
                    <div>
                        <CardTitle className="text-xl">Create New Vote</CardTitle>
                    </div>
                </div>
            </CardHeader>
                                ...
                                <TabsContent value="member" className="space-y-2 mt-0">
                                    <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                                        {selectedCandidateIds.map((userId, idx) => {
                                            const member = nonAdminMembers.find(m => m.userId === userId);
                                            return (
                                                                        ...
                                                                        <SelectItem key={m.userId} value={m.userId}>
                                                                            <div className="flex items-center gap-2">
                                                                                <Avatar className="h-6 w-6">
                                                                                    <AvatarImage src={m.user.image || "/placeholder.svg"} />
                                                                                    <AvatarFallback>
                                                                                        {(m.user.name ? m.user.name.split(" ").map((n: string) => n[0]).join("") : "?")}
                                                                                    </AvatarFallback>
                                                                                </Avatar>
                                                                                <div className="flex flex-col items-start gap-0.5">
                                                                                    <span className="text-sm font-medium leading-none">{m.user.name || "Unnamed"}</span>
